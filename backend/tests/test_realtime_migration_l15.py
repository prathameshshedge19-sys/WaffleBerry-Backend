...
def test_0016_fresh_upgrade_downgrade_reupgrade_preserves_history(tmp_path, existing):
    ...
    assert "0016_realtime_sessions (head)" in alembic(path, "current")
    with engine.connect() as c:
...
def snapshot(connection):
    return {name: connection.exec_driver_sql('SELECT * FROM "'+name+'"').fetchall()
            for name in sa.inspect(connection).get_table_names() if name not in {"alembic_version","realtime_sessions"}}
