...
Final production Story acceptance remains required: an authenticated disposable owner/visitor setup must run memory-only, perspective, grounding, edit/version, regeneration, staleness, publication, visitor, collaborator and cross-Legacy checks, followed by synthetic cleanup. The supported authentication audit found only public signup → emailed OTP → verified registration → login; no documented QA account, operator/test-account command, catchall mailbox or sanctioned verification path exists. An external verification mailbox is therefore required. No disposable production Story data was created and no customer data was touched. The release is stopped before tagging; no direct database fixture, auth-table write or minted token was used.
