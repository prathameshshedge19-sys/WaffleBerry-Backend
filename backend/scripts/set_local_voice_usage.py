"""Set today's Berry voice usage for local acceptance testing only."""
...
def main() -> None:
    ...
    with SessionLocal() as db:
        ...
        limit = get_plan_limits(user.plan).voice_plays_daily
        if args.used > limit:
            parser.error(f"--used cannot exceed this user's limit ({limit})")
        row = QuotaService(db).get_daily_usage(user)
        row.voice_plays = args.used
        db.commit()
        print(f"Set today's Berry voice usage to {args.used}/{limit} for {user.email}.")
